...
impl Cmd {
  pub fn new(input: &str) -> Self {
    ...
    let (stdout_redirect, stderr_redirect, stdin_redirect) = Cmd::ensure_redirect(&args).unwrap();
          ...
          if self.executable_exists() {
            ...
            if let Some(stdout_info) = &self.stdout_redirect {
              let file = match stdout_info.mode {
                RedirectMode::Write | RedirectMode::ForceWrite => File::create(&stdout_info.path)?,
            ...
            if let Some(stderr_info) = &self.stderr_redirect {
              let file = match stderr_info.mode {
                RedirectMode::Write | RedirectMode::ForceWrite => File::create(&stderr_info.path)?,
                RedirectMode::Append => OpenOptions::new()
                  .write(true)
                  .append(true)
                  .create(true)
