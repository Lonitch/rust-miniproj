...
fn test_parse_valid_config() {
    ...
    assert_eq!(config.settings.unwrap().max_connections.unwrap(), 100);
    assert_eq!(
        config.features,
...
fn test_case_insensitive_mode() {
    ...
    let config =
        parse_config(&json_data).expect("Failed to parse config with case-insensitive mode");
    assert_eq!(config.mode, Some(Mode::Development));
}
...
fn test_default_settings() {
    ...
    assert_eq!(
        config.settings.unwrap().theme.unwrap(),
        "default".to_string()
    );
}
