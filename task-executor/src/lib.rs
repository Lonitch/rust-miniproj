...
pub fn generate_payloads(num: u32) -> Vec<Payload> {
    ...
    for _ in 0..num {
        let url = format!(
            ...
            (0..8).map(|_| rand::random::<char>()).collect::<String>()
        );
...
impl ApiConfig {
    ...
    pub async fn simulate_api_call(&self, payload: &Payload) -> TaskResult {
        ...
        let sleep_fut =
            tokio::time::sleep(Duration::from_secs_f32(rand::random::<f32>() * base_delay));
        let result = match tokio::time::timeout(
            Duration::from_secs_f32(base_delay / 2.0),
            sleep_fut,
        )
        .await
        {
